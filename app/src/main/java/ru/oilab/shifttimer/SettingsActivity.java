...
public class SettingsActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        root.addView(salaryInput, margins(dp(49), 0, 0, dp(3)));

        TextView hint = text("Полный месяц равен указанной сумме.",
                11, 0xFF6E7580, false);
        hint.setPadding(dp(4), dp(2), 0, dp(13));
        root.addView(hint);

        root.addView(label("ФОН ВИДЖЕТА"));
        photoStatus = text("", 12, 0xFF87909C, false);
        photoStatus.setPadding(dp(4), 0, 0, dp(5));
        ...
        root.addView(photoRow, margins(dp(41), 0, 0, dp(14)));
        ...
        root.addView(save, margins(dp(48), 0, 0, 0));
    ...
    private void refreshPhotoStatus() {
        ...
        photoStatus.setText(ShiftPreferences.hasWidgetPhoto(this)
                ? "Фото выбрано — подгоняется под размер виджета."
                : "Без фото — тёмный фон.");
    }
